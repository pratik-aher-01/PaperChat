...
class PerplexityParser(BaseParser):
    """Parse Perplexity shared search conversations from rendered HTML."""
    ...
    def parse(self, fetch_result: FetchResult) -> Conversation:
        ...
        if not messages:
            raise ParserException("No Perplexity messages were found in the fetched HTML.")
    ...
    def _extract_messages(self, soup: BeautifulSoup) -> Iterable[Message]:
        """Extract user queries and perplexity answers from DOM elements."""
    ...
    def _extract_fallback_messages(self, soup: BeautifulSoup) -> Iterable[Message]:
        """Fallback message extraction for Perplexity."""
        # Find H1 or query heading as user message
        h1 = soup.select_one("h1")
        index = 1
        seen: set[str] = set()

        if isinstance(h1, Tag):
            query_text = h1.get_text(" ", strip=True)
            if query_text:
                seen.add(f"user:{query_text[:300]}")
                yield Message(
                    id="perplexity-user-1",
                    role="user",
                    plain_text=query_text,
                    content_blocks=(ContentBlock(type="markdown", text=query_text),),
                )
                index += 1

        # Look for markdown answer blocks
        for container in soup.select(".ds-markdown, article, section"):
            if not isinstance(container, Tag):
                continue
            text = _node_to_markdown(container)
            if not text or len(text) < 5:
                continue
            key = f"assistant:{text[:300]}"
            if key in seen:
                continue
            seen.add(key)
            yield Message(
                id=f"perplexity-assistant-{index}",
                role="assistant",
                plain_text=text,
    ...
    def _find_content_node(self, node: Tag) -> Tag:
        """Find inner content container."""
        for selector in (".ds-markdown", ".prose", ".answer-content"):
...
def _extract_title(soup: BeautifulSoup, fallback: str) -> str:
    ...
    if soup.title and soup.title.string:
        clean = soup.title.string.replace("- Perplexity", "").replace("Perplexity -", "").strip()
        if clean:
            return clean
...
def _message_id(node: Tag, index: int) -> str:
    """Generate message ID."""
    for attr in ("id", "data-message-id", "data-testid"):
        val = node.get(attr)
        if isinstance(val, str) and val.strip():
            return val.strip()
    return f"perplexity-msg-{index}-{uuid4().hex[:8]}"


def _extract_messages_from_embedded_data(soup: BeautifulSoup) -> Iterable[Message]:
    """Extract messages from inline script payloads."""
    ...
    for script in soup.find_all("script"):
        ...
        if not text or ("__NEXT_DATA__" not in text and "query" not in text and not text.strip().startswith("{")):
            continue
            ...
            if isinstance(item, dict):
                ...
                if role and text_val:
                    ...
                    if key not in seen:
                        ...
                        messages.append(
                            Message(
                                id=f"perplexity-json-{len(messages)+1}",
                                role=role,
                                plain_text=text_val,
...
def _json_role(data: dict[object, object]) -> str | None:
    """Determine message role from JSON dict."""
    for key in ("role", "author", "type", "mode"):
        val = data.get(key)
        if isinstance(val, str):
            val_lower = val.lower()
            if "query" in val_lower or "user" in val_lower or "human" in val_lower:
                return "user"
            if "answer" in val_lower or "assistant" in val_lower or "bot" in val_lower or "perplexity" in val_lower:
                return "assistant"
    return None


def _json_text(data: dict[object, object]) -> str:
    """Extract markdown text from JSON dict."""
    for key in ("query", "answer", "text", "content"):
        val = data.get(key)
        if isinstance(val, str) and val.strip():
            return val.strip()
    return ""
...
def _walk_json_payloads(text: str) -> Iterable[object]:
    """Search text for inline JSON payloads."""
    start = 0
    while True:
        idx = text.find("{", start)
        if idx == -1:
...
def _node_to_markdown(node: Tag) -> str:
    """Convert HTML node tree to Markdown."""
