...
class ClaudeParser(BaseParser):
    """Parse Claude shared conversations from rendered HTML."""
    ...
    def parse(self, fetch_result: FetchResult) -> Conversation:
        ...
        if not messages:
            raise ParserException("No Claude messages were found in the fetched HTML.")
    ...
    def _extract_messages(self, soup: BeautifulSoup) -> Iterable[Message]:
        """Extract user and assistant messages from Claude DOM elements."""
        ...
        # Find tags matching user message selectors
        for selector in self.user_selectors:
            for tag in search_root.select(selector):
                if isinstance(tag, Tag):
                    candidates.append((_node_offset(tag), tag, "user"))

        # Find tags matching assistant message selectors
        for selector in self.assistant_selectors:
            for tag in search_root.select(selector):
                if isinstance(tag, Tag):
                    # Check if node explicitly specifies user
                    role = "user" if tag.get("data-is-user") == "true" else "assistant"
                    candidates.append((_node_offset(tag), tag, role))
    ...
    def _extract_fallback_messages(self, soup: BeautifulSoup) -> Iterable[Message]:
        """Fallback message parser using generic containers."""
        ...
        for article in articles:
            ...
            if not text or len(text) < 5:
                continue
            role = "user" if "user" in str(article.get("class", [])) else "assistant"
            key = f"{role}:{text[:300]}"
            if key in seen:
                continue
            seen.add(key)
            yield Message(
                id=f"claude-fallback-{index}",
                role=role,
                plain_text=text,
    ...
    def _find_content_node(self, node: Tag) -> Tag:
        """Find inner markdown container if present."""
        for selector in (".font-claude-message", ".grid", ".contents", "div.grid"):
...
def _node_offset(node: Tag) -> int:
    """Calculate approximate line offset in source."""
    return node.sourceline if node.sourceline is not None else 0


def _extract_title(soup: BeautifulSoup, fallback: str) -> str:
    """Extract Claude conversation title."""
    if soup.title and soup.title.string:
        clean = soup.title.string.replace("- Claude", "").replace("Claude -", "").strip()
        if clean:
            return clean
    heading = soup.select_one("h1, [data-testid*='title']")
...
def _message_id(node: Tag, index: int) -> str:
    ...
    for attr in ("id", "data-message-id", "data-testid"):
        ...
        if isinstance(val, str) and val.strip():
            return val.strip()
    return f"claude-msg-{index}-{uuid4().hex[:8]}"


def _extract_messages_from_embedded_data(soup: BeautifulSoup) -> Iterable[Message]:
    """Extract messages from embedded __NEXT_DATA__ or script payloads."""
    ...
    for script in soup.find_all("script"):
        ...
        if not text or ("__NEXT_DATA__" not in text and "chat_messages" not in text and not text.strip().startswith("{")):
            continue
            ...
            if isinstance(item, dict):
                ...
                if role and text_val:
                    ...
                    if key not in seen:
                        ...
                        messages.append(
                            Message(
                                id=f"claude-json-{len(messages)+1}",
                                role=role,
                                plain_text=text_val,
...
def _json_role(data: dict[object, object]) -> str | None:
    """Determine role from JSON data."""
    for key in ("sender", "role", "author", "type"):
        val = data.get(key)
        if isinstance(val, str):
            val_lower = val.lower()
            if "user" in val_lower or "human" in val_lower:
                return "user"
            if "assistant" in val_lower or "claude" in val_lower:
                return "assistant"
    return None
        ...
        if isinstance(val, list):
            ...
            for elem in val:
                if isinstance(elem, str) and elem.strip():
                    ...
                    txt = elem.get("text") or elem.get("content")
...
def _walk_obj(obj: object) -> Iterable[object]:
    """Recursively walk nested JSON structure."""
    yield obj
